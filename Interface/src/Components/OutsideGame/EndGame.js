                  ...
                  { this.state.finalPlayers.map((entry, index) => (
                    <Flexbox key={ index }>
                      <Flexbox flexGrow={ 1 } alignItems='center' justifyContent='flex-start'>
                        ...
                        <font style={ { marginLeft: '10px' } }> { entry.name } </font>
                      </Flexbox>
                      <Flexbox alignItems='center'>
                        <div> { svgIcon('coinDollar') } </div>
                        <font style={ { marginLeft: '10px' } } color={ entry.politicalCapital > 0 ? colors.GREEN : colors.RED }> { entry.politicalCapital } </font>
                      </Flexbox>
                    </Flexbox>
                  ))
                  }
                    ...
                    { this.state.finalParties.map((entry, index) => (
                      <Flexbox flexDirection='column' flexGrow={ 1 } key={ index } >
                        <Flexbox flexGrow={ 1 } justifyContent='flex-start' alignItems='center'>
                          <Flexbox flexGrow={ 1 } alignItems='center' justifyContent='flex-start'>
                            <div> { this.renderPlace(index + 1) } </div>
                            <font style={ { marginLeft: '10px' } }> { entry.partyName } </font>
                          </Flexbox>
                          <Flexbox alignItems='center'>
                            <div> { svgIcon('coinDollar') } </div>
                            <font style={ { marginLeft: '10px' } } color={ entry.totalPoliticalCapital > 0 ? colors.GREEN : colors.RED }> { entry.totalPoliticalCapital } </font>
                          </Flexbox>
                        </Flexbox>
                        <Flexbox flexWrap='wrap' style={ { marginLeft: '15px' } }>
                          { entry.players.map((player, index2) => (
